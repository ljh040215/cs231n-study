...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        '''
        # 1. 图像特征处理：投影到词向量维度，并扩展为序列形式（适配解码器输入）
        memory = self.visual_projection(features)  # (N, D) → (N, W)
        memory = memory.unsqueeze(1)  # (N, W) → (N, 1, W)（S=1，源序列长度为1）

        # 2. 描述文本处理：词嵌入 + 位置编码
        tgt_embed = self.embedding(captions)  # (N, T) → (N, T, W)
        tgt_embed = self.positional_encoding(tgt_embed)  # 添加位置编码，形状仍为 (N, T, W)

        # 3. 构建目标序列掩码（下三角掩码）：屏蔽未来时间步
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device))  # (T, T)，下三角为1，上三角为0
        tgt_mask = tgt_mask.masked_fill(tgt_mask == 0, float('-inf')).masked_fill(tgt_mask == 1, float(0.0))

        # 4. Transformer解码器前向传播
        decoder_out = self.transformer(tgt=tgt_embed, memory=memory, tgt_mask=tgt_mask)  # (N, T, W)

        # 5. 输出投影：映射到词汇表得分
        scores = self.output(decoder_out)  # (N, T, W) → (N, T, V)
        '''
