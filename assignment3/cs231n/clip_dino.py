...
import os
class DavisDataset:
    def __init__(self, root='/root/autodl-tmp/cs231n/data/DAVIS'):
        self.root = root
        self.img_root = os.path.join(root, 'JPEGImages', '480p')
        self.mask_root = os.path.join(root, 'Annotations', '480p')
        self.videos = sorted(os.listdir(self.img_root))
        # self.videos = sorted(os.listdir(os.path.join(root, 'JPEGImages')))
    ...
    def get_sample(self, index):
        ...
        frames = [np.array(Image.open(os.path.join(img_dir, f))) for f in frame_files]
